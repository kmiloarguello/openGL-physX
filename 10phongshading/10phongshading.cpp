...
int shader_choice = 0;
glm::vec4 lightposition = glm::vec4(0,1,0, 1);
...
void processEvents(SDL_Event current_event)
{
	...
	case SDL_KEYDOWN:
	{
		if (current_event.key.keysym.sym == SDLK_ESCAPE)
			quit = true;
		if (current_event.key.keysym.sym == SDLK_1)
			shader_choice = 0;
		if (current_event.key.keysym.sym == SDLK_2)
			shader_choice = 1;
		if (current_event.key.keysym.sym == SDLK_3)
			shader_choice = 2;
...
int main(int argc, char *argv[])
{
	...
	// Create window
	window = SDL_CreateWindow("PhongShading-Drawing", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, window_width, window_height, SDL_WINDOW_OPENGL | SDL_WINDOW_RESIZABLE);
	...
	vector<myShader *> shaders (4);
	myShader *shader;
	shaders[0] = new myShader("phong-vertexshader.glsl", "phong-fragmentshader.glsl");
	shaders[1] = new myShader("goraud-vertexshader.glsl", "goraud-fragmentshader.glsl");
	shaders[2] = new myShader("flat-vertexshader.glsl", "flat-fragmentshader.glsl");
	shaders[3] = new myShader("basic-vertexshader.glsl", "basic-fragmentshader.glsl");
	...
	while (!quit)
	{
		...
		glUniform4fv(glGetUniformLocation(shader->shaderprogram, "light_position"), 1, glm::value_ptr(lightposition));
		/*********************************************************************/
		...
		shaders[3]->start();
		glUniformMatrix4fv(glGetUniformLocation(shaders[3]->shaderprogram, "myview_matrix"), 1, GL_FALSE, glm::value_ptr(view_matrix));
		glUniformMatrix4fv(glGetUniformLocation(shaders[3]->shaderprogram, "myprojection_matrix"), 1, GL_FALSE, glm::value_ptr(projection_matrix));
		glBegin(GL_POINTS);
