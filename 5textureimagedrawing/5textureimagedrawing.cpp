...
int main(int argc, char *argv[])
{
	...
	// Create window
	window = SDL_CreateWindow("TextureImage-Drawing", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, window_width, window_height, SDL_WINDOW_OPENGL | SDL_WINDOW_RESIZABLE);
	...
	vertices.push_back(glm::vec3(0.5, -0.5, 0.0f));
	vertices.push_back(glm::vec3(0.5, 0.5, 0.0f));
	vertices.push_back(glm::vec3(-0.5, 0.5, 0.0f));
	...
	GLuint texture_id;
	{
		...
		GLubyte *mytexture = stbi_load("scenary.jpg", &width, &height, &size, 4);
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, static_cast<GLuint>(width), 
					static_cast<GLuint>(height), 0, GL_RGBA, GL_UNSIGNED_BYTE, mytexture);
