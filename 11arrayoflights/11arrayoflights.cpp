...
int main(int argc, char *argv[])
{
	...
	// Create window
	window = SDL_CreateWindow("PassingArraystoShaders-Drawing", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,	window_width, window_height, SDL_WINDOW_OPENGL | SDL_WINDOW_RESIZABLE);
	...
	myShader *basic_shader = new myShader("basic-vertexshader.glsl", "basic-fragmentshader.glsl");
	myShader *phong_shader = new myShader("phong-vertexshader.glsl", "phong-fragmentshader.glsl");

	/*********************************************************************/
	vector<glm::vec4> lightpositions;
	lightpositions.push_back(glm::vec4(0, 0.6, 0, 1));
	lightpositions.push_back(glm::vec4(0, -0.6, 0, 1));
	lightpositions.push_back(glm::vec4(1.0, 0, 0, 1));
	/*********************************************************************/
	...
	while (!quit)
	{
		...
		glUniformMatrix4fv(glGetUniformLocation(phong_shader->shaderprogram, "myprojection_matrix"), 1, GL_FALSE, glm::value_ptr(projection_matrix));

		glm::mat4 view_matrix = glm::lookAt(camera_eye, camera_eye + camera_forward, camera_up);
		glUniformMatrix4fv(glGetUniformLocation(phong_shader->shaderprogram, "myview_matrix"), 1, GL_FALSE, glm::value_ptr(view_matrix));

		glm::mat3 normal_matrix = glm::transpose(glm::inverse(glm::mat3(view_matrix)));
		glUniformMatrix3fv(glGetUniformLocation(phong_shader->shaderprogram, "mynormal_matrix"), 1, GL_FALSE, glm::value_ptr(normal_matrix));

		/*********************************************************************/
		tmp.clear();
		for (int i = 0; i < lightpositions.size(); i++) for (int j : {0, 1, 2, 3}) tmp.push_back(lightpositions[i][j]);
		glUniform4fv(glGetUniformLocation(phong_shader->shaderprogram, "light_positions"), lightpositions.size(), &tmp[0]);
		glUniform1i(glGetUniformLocation(phong_shader->shaderprogram, "num_lights"), lightpositions.size());
		/*********************************************************************/
		...
		basic_shader->start();
		glUniformMatrix4fv(glGetUniformLocation(basic_shader->shaderprogram, "myview_matrix"), 1, GL_FALSE, glm::value_ptr(view_matrix));
		glUniformMatrix4fv(glGetUniformLocation(basic_shader->shaderprogram, "myprojection_matrix"), 1, GL_FALSE, glm::value_ptr(projection_matrix));
		glBegin(GL_POINTS);
		for (int i = 0; i < lightpositions.size(); i++)
			glVertex3fv(&lightpositions[i][0]);
		glEnd();
